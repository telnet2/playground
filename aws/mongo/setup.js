...
const debug_ssh = require("debug")("ssh");
...
function connect(host) {
    ...
    return new Promise((resolve, reject) => {
        ...
        console.log(
            `[${host.serviceName}]`,
            "connecting to ",
            JSON.stringify(_.pick(conx, "host", "user", "serviceName"))
        );

        ssh.openShell = function openShell() {
            return new Promise((resolve, reject) => {
...
if (require.main === module) {
    //#    program.arguments("<jsOrJsonFile...>").action(main);
    program
        .requiredOption("-f, --file <file>", "config file")
        .option("--install", "Copy required files to the server")
        .option("--scripts", "Run init scripts on the server")
        .option("-d, --dryrun", "Print contents of files not copy")
        .action((...args) => {
            try {
                setupMongo(...args);
            } catch (err) {
